    ...
    return (
        <div className='bg-[--gray-200] rounded p-4 hover:bg-[#808080] hover:bg-opacity-40 text-sm relative lg:w-[40%] w-[80%] mb-12 last:mb-0'>
            <h3 className='flex items-center gap-1'>
                ...
                height={10} 
                width={10} 
                src={small.startsWith('http') ? small : '/missing_small.png'}
                alt={name} 
                className='rounded-full w-5 h-5'></Image>
            </h3>
            ...
            <Image
                height={10}
                width={100}
                src={large.startsWith('http') ? large : '/missing_large.png'}
                alt={name}
                className='rounded-full md:w-32 w-20 h-auto absolute -right-6 -top-8 md:-right-10 md:-top-2 z-10'></Image>
        </div>
    );
};
